...
from argus.scn.diff import (
    _build_generic_resource,
    determine_operation,
    extract_changed_attributes,
...
class TestExtractChangedAttributes:
    ...
    def test_terraform_style_attributes(self):
        ...
        attrs = extract_changed_attributes(diff, 0)
    ...
    def test_empty_diff(self):
        ...
        assert extract_changed_attributes("", 0) == []
...
class TestParseTerraformDiff:
    ...
    def test_extracts_resource_block(self):
        ...
        result = parse_terraform_diff("main.tf", diff)
    ...
    def test_fallback_to_generic_resource(self):
        ...
        diff = "+variable = 42"
        result = parse_terraform_diff("vars.tf", diff)
        assert len(result['resources']) == 1
        assert result['resources'][0]['type'] == 'unknown'

    def test_empty_diff(self):
        """Empty diff -> no resources."""
        result = parse_terraform_diff("main.tf", "")
...
class TestParseKubernetesDiff:
    ...
    def test_extracts_kind_and_name(self):
        ...
        result = parse_kubernetes_diff("deploy.yaml", diff)
    ...
    def test_unnamed_resource(self):
        ...
        diff = "kind: Service\n"
        result = parse_kubernetes_diff("svc.yaml", diff)
        assert result['resources'][0]['name'] == 'unnamed'

    def test_multiple_documents(self):
        """Multiple kind entries -> multiple resources."""
        diff = "kind: Deployment\nname: app\n---\nkind: Service\nname: svc\n"
        result = parse_kubernetes_diff("all.yaml", diff)
        assert len(result['resources']) == 2

    def test_fallback_to_generic(self):
        """No kind pattern -> generic resource."""
        diff = "+replicas: 3"
        result = parse_kubernetes_diff("config.yaml", diff)
        assert result['resources'][0]['type'] == 'unknown'
...
class TestParseCloudFormationDiff:
    ...
    def test_yaml_format(self):
        ...
        result = parse_cloudformation_diff("template.yaml", diff)
    ...
    def test_json_format(self):
        ...
        result = parse_cloudformation_diff("template.json", diff)
    ...
    def test_empty_diff(self):
        """Empty diff -> no resources."""
        result = parse_cloudformation_diff("template.yaml", "")
        assert result['resources'] == []
