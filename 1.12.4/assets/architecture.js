...
(function () {
  ...
  // SDK scanner / linter name → composite-action directory under
  // ``.github/actions/``. Most SDK scanners map 1:1 onto a
  // ``scanner-<name>`` action (the implicit default below), but the
  // container family (Trivy + Grype + Syft) is bundled into a single
  // ``scanner-container`` composite and a couple of actions don't
  // carry the ``scanner-`` prefix at all. Without this table the
  // generated workflow references actions that don't exist.
  const SDK_TO_ACTION = {
    'trivy': 'scanner-container',
    'grype': 'scanner-container',
    'syft': 'scanner-container',
    'trivy-iac': 'scanner-trivy-iac',
    'scn-detector': 'scn-detector',
  };

  function actionForScanner(name) {
    if (name.startsWith('lint-')) {
      return `linter-${name.slice('lint-'.length)}`;
    }
    return SDK_TO_ACTION[name] || `scanner-${name}`;
  }
  ...
  function buildGithubWorkflow(names) {
    ...
    const lines = [];
    // Security-policy reminder up top: Argus repos SHA-pin every
    // ``uses:`` reference. The starter snippet below uses tag refs
    // so the version stays human-readable; pinact, Dependabot, or
    // ``gh actions-runner`` can resolve them to commit SHAs before
    // the workflow ships to production.
    lines.push('# Generated by the Argus architecture map.');
    lines.push('# Before deploying, SHA-pin every ``uses:`` reference');
    lines.push('# (project policy — protects against tag-rewriting');
    lines.push('# supply-chain attacks). Tools like ``pinact`` or');
    lines.push('# Dependabot can do this in a follow-up commit.');
    lines.push('jobs:');
    lines.push('  argus:');
    lines.push('    runs-on: ubuntu-latest');
    lines.push('    steps:');
    lines.push('      - uses: actions/checkout@v5');
    // Several SDK scanners collapse to one composite action (e.g.
    // trivy / grype / syft all run inside scanner-container). Track
    // emitted action paths so the workflow doesn't duplicate steps.
    const seen = new Set();
    for (const name of names) {
      const action = actionForScanner(name);
      ...
      lines.push(`      - uses: huntridge-labs/argus/.github/actions/${action}@${ref}`);  // release-it-ignore
      if (state.severityThreshold && state.severityThreshold !== 'none') {
        lines.push('        with:');
        lines.push(`          fail_on_severity: ${state.severityThreshold}`);
      }
    }
    return lines.join('\n') + '\n';
  }
