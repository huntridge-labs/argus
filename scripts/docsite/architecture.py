...
def render_inline_markdown(
    ...
    md = (
        ...
        "\n"
        "<style>\n"
        "/* MkDocs Material sets ``html { font-size: 125% }`` (i.e.\n"
        "   ~22 px / rem) for accessibility scaling. Our architecture\n"
        "   CSS is designed against the browser default of 16 px / rem,\n"
        "   so without this reset every ``rem`` measurement in\n"
        "   ``architecture.css`` (sidebar widths, viewport padding,\n"
        "   spacing) is 37.5 % too big — the diagram ends up boxed\n"
        "   into a narrow strip. Pin html back to 16 px on this page\n"
        "   only; the architecture surface owns its own scaling. */\n"
        "html { font-size: 16px; }\n"
        "\n"
