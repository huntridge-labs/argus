...
<head>
  ...
  <title>Architecture &amp; flows — Argus</title>
...
<body>
  ...
  <footer>
    Generated by <a href="https://argus.huntridgelabs.com">argus</a>
  </footer>

  <script id="arch-data" type="application/json">__VIEW_MODEL__</script>
...
def render_standalone_html(
    ...
    """Render the standalone architecture page into ``output_dir``.

    Writes ``index.html`` with the view model inlined as a JSON
    script block, and copies ``argus.css`` + ``architecture.css`` +
    ``architecture.js`` from the browser-viewer's static directory
    sideways. The result is genuinely self-contained: open
    ``output_dir/index.html`` directly from disk and the page
    renders with no network requests.

    Args:
        view_model: Output of ``build_view_model`` — JSON-serializable.
        output_dir: Where to write the page. Created if missing.
    ...
    # without that asset).
    for name in ("argus.css", "architecture.css", "architecture.js"):
...
def _render_legend(columns: list[dict[str, Any]]) -> str:
    """Build the top legend HTML directly (no Jinja roundtrip)."""
    import html as _html
    parts: list[str] = []
    for col in columns:
        swatch = _html.escape(str(col.get("swatch", "muted")))
        label = _html.escape(str(col.get("label", "")))
        parts.append(
            f'<span class="arch-legend__item">'
            f'<span class="arch-legend__swatch arch-legend__swatch--{swatch}"></span>'
            f'{label}'
            f'</span>'
        )
    return "".join(parts)
