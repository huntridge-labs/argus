#!/usr/bin/env bash
# Build the MUMPS tree-sitter grammar for local execution of `argus scan mumps`.
#
...
# scanner-mumps container image need to compile it once. This script does
...
#
# Override with ARGUS_MUMPS_GRAMMAR_OUT to write somewhere else.
set -euo pipefail

# Pinned to MITRE Public Release 23-4084. Update in lockstep with
# docker/Dockerfile.mumps and .ai/architecture.yaml.
...
OUT_PATH="${ARGUS_MUMPS_GRAMMAR_OUT:-${HOME}/.cache/argus/grammars/mumps.so}"
