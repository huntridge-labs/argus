...
class TestParseGrypeResults:
    ...
    def test_check_error_clean_scan(self):
        ...
        result = parse_grype.check_scan_error(str(FIXTURES_DIR / "results-zero-findings.json"))
        assert result == ""

    def test_check_error_with_findings(self):
        """Test check-error returns empty for results with findings."""
        result = parse_grype.check_scan_error(str(FIXTURES_DIR / "results-with-findings.json"))
        assert result == ""

    def test_check_error_scan_failed(self):
        """Test check-error detects scan error marker."""
        result = parse_grype.check_scan_error(str(FIXTURES_DIR / "results-scan-error.json"))
        assert result != ""
        assert "scan failed" in result.lower() or "unable to pull" in result.lower()

    def test_check_error_nonexistent_file(self):
        """Test check-error with nonexistent file."""
        result = parse_grype.check_scan_error("/nonexistent/file.json")
        assert result == ""
    ...
    def test_check_error_empty_file(self, tmp_path):
        ...
        empty.write_text("")
        result = parse_grype.check_scan_error(str(empty))
        assert result == ""
    ...
    def test_check_error_without_message(self, tmp_path):
        ...
        f.write_text(json.dumps({"scan_error": True, "matches": []}))
        result = parse_grype.check_scan_error(str(f))
    ...
    def test_check_error_false_marker(self, tmp_path):
        ...
        f.write_text(json.dumps({"scan_error": False, "matches": []}))
        result = parse_grype.check_scan_error(str(f))
        assert result == ""
    ...
    def test_check_error_cli(self):
        ...
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=10)
        assert result.returncode == 0
        assert "scan failed" in result.stdout.lower() or "unable to pull" in result.stdout.lower()
