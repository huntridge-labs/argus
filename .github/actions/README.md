...
steps:
  ...
  - name: Run Security Scanner
    uses: huntridge-labs/argus/.github/actions/scanner-{name}@1.7.0
    env:
      GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
    with:
...
```yaml
- uses: actions/checkout@v6
- uses: huntridge-labs/argus/.github/actions/scanner-trivy-iac@1.7.0
  env:
    GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
  with:
    iac_path: 'terraform'
...
- uses: huntridge-labs/argus/.github/actions/scanner-container@1.7.0
  env:
    GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
  with:
...
```yaml
- uses: actions/checkout@v6
- uses: huntridge-labs/argus/.github/actions/linter-yaml@1.7.0
  with:
...
```yaml
- uses: actions/checkout@v6
- uses: huntridge-labs/argus/.github/actions/linter-python@1.7.0
  with:
    fail_on_issues: false
    max_line_length: '120'
```
...
```yaml
- uses: actions/checkout@v6
- uses: huntridge-labs/argus/.github/actions/linter-dockerfile@1.7.0
  with:
    fail_on_issues: false
```
...
jobs:
  security-scan:
    ...
    steps:
      - uses: actions/checkout@v6
      - uses: huntridge-labs/argus/.github/actions/scanner-${{ matrix.scanner }}@1.7.0
        env:
          GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
```
