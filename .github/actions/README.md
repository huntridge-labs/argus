...
steps:
  ...
  - name: Run Security Scanner
    uses: huntridge-labs/argus/.github/actions/scanner-{name}@0.7.2
    env:
      GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
    with:
...
```yaml
- uses: actions/checkout@v6
- uses: huntridge-labs/argus/.github/actions/scanner-trivy-iac@0.7.2
  env:
    GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
  with:
    iac_path: 'terraform'
...
```yaml
- uses: actions/checkout@v6
- uses: huntridge-labs/argus/.github/actions/linter-yaml@0.7.2
  with:
...
```yaml
- uses: actions/checkout@v6
- uses: huntridge-labs/argus/.github/actions/linter-python@0.7.2
  with:
    fail_on_issues: false
    max_line_length: '120'
```
...
```yaml
- uses: actions/checkout@v6
- uses: huntridge-labs/argus/.github/actions/linter-dockerfile@0.7.2
  with:
    fail_on_issues: false
```
...
jobs:
  security-scan:
    ...
    steps:
      - uses: actions/checkout@v6
      - uses: huntridge-labs/argus/.github/actions/scanner-${{ matrix.scanner }}@0.7.2
        env:
          GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
```
