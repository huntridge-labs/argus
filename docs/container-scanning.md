...
```yaml
# yaml-language-server: $schema=https://raw.githubusercontent.com/huntridge-labs/argus/1.12.4/.github/actions/parse-container-config/schemas/container-config.schema.json
```
...
jobs:
  scan:
    uses: huntridge-labs/argus/.github/workflows/container-scan-from-config.yml@1.12.4
    with:
