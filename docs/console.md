...
The **Argus Console** is a home base for the whole local workflow — run a
scan, browse findings, edit config, initialize a project, and tune your
preferences, all without leaving the terminal. It is an **Argus add-on**:
with it installed, running `argus` with no subcommand in an interactive
terminal opens the Console.

![Argus Console home — wordmark, project status line, and the launcher menu](images/console/console-home.png)

```bash
argus            # opens the Console when the add-on is installed
argus view       # always jumps straight to the findings viewer
argus scan ...   # every subcommand is unchanged
```

> **Backward compatible by design.** The Console only takes over bare `argus`
> when stdout and stdin are a TTY *and* a Console provider is installed. Piped,
> redirected, or CI invocations keep printing `--help`. On a stock open-source
> install (no provider), bare `argus` prints a one-line pointer and falls back
> to `--help`, so scripts and pipelines are unaffected.

> **Get the Console.** It ships as an Argus add-on that plugs into the
> open-source core through the `argus.console_providers` seam. Learn more or
> request access: <https://www.huntridgelabs.com/>.
