...
| `vex` | string \| list[string] | `container` | Path(s) to OpenVEX document(s), passed to both trivy and grype (`--vex`) so `not_affected` / `fixed` findings are filtered at the source with an auditable justification. Accepts a single path or a list. Equivalent CLI flag: `--vex PATH` (repeatable). Missing files are logged and skipped (findings stay visible). Pairs with the OpenVEX doc produced by the `argus view` triage screen. |
