...
```yaml
# yaml-language-server: $schema=https://raw.githubusercontent.com/huntridge-labs/argus/1.10.0/argus-config.schema.json
version: "1.0"
```
...
| `vex` | string \| list[string] | `container`, `grype`, `trivy` | Path(s) to OpenVEX document(s) so `not_affected` / `fixed` findings are filtered at the source (`--vex`) with an auditable justification. Any VEX-capable scanner (`supports_vex`) reads it: `containers.vex` (or `argus scan container --vex PATH`) for the container scan; `scanners.grype.vex` / `scanners.trivy.vex` for the source/SBOM scans. Accepts a single path or a list. Missing files are logged and skipped (findings stay visible). Pairs with the OpenVEX doc produced by the `argus view` triage screen. |
