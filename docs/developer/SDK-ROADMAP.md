...
- [ ] **scanner-zap target setup parity — decided** (see ADR-024 in `.ai/decisions.yaml`).
  ...
  **Implementation tasks** (separate PRs, all gated by ADR-024):
  - [x] ~~Schema: extend `ScannerConfig` and `docs/config-reference.md` with the new `scanners.zap.*` keys plus the `scanners.zap.auth.*` sub-block.~~ Shipped. `argus/core/schema.py` allowlists `api_spec`, `rules_file`, `cmd_options`, `max_duration_minutes`, `healthcheck_url`, `app_image_ref`, `app_ports`, and the nested `auth` block with dedicated validation in `_validate_zap_auth`; `docs/config-reference.md` documents every key in the zap rows of the config table.
  - [x] ~~Validator: env-var-interpolation requirement for `registry_password`, `auth.password_env`, `auth.username_env` — warn on literal values at config-load time, not at scan time.~~ Shipped. Registry credentials enforced via the shared `CREDENTIAL_FIELDS` table in `schema.py`; `auth.username_env` / `auth.password_env` validated by `_validate_zap_auth` with literal-value warnings emitted at config-load.
  - [x] ~~`argus/scanners/zap.py`: read the new config keys, build the ZAP CLI with `rules_file` / `cmd_options` / `max_duration_minutes` / `api_spec` / context-file mount.~~ Shipped. The scanner reads every key (`api_spec`, `rules_file`, `cmd_options`, `max_duration_minutes`, `auth.context_file`) and mounts `rules_file` + `auth.context_file` into the container.
  - [ ] Honor `healthcheck_url` with a pre-scan poll. Key is schema-allowlisted but not yet consumed in `argus/scanners/zap.py` — the engine should poll the URL for a 2xx response before invoking ZAP (configurable timeout, default 60s) and fail fast with a clear error if the SUT never becomes ready. Needs scanner wiring + unit tests + a doc snippet in `docs/config-reference.md`.
  - [x] ~~CLI: `--registry-password-stdin` flag, wired through to the engine for both `zap` and `container`.~~ Shipped in PR #145 (and the parallel `--zap-auth-password-stdin` for web-app auth). Stdin reads once, slot-registry in `argus/core/secrets.py` (`set_stdin_override` / `get_stdin_override`), highest precedence over `<field>_env` and literal `<field>`. Value never reaches per-scanner config dict, so audit/log paths can't leak it.
  - [x] ~~Composite action trim: remove `api_spec`, `rules_file_name`, `cmd_options`, `max_duration_minutes`, `healthcheck_url`, `registry_username`, `registry_password` from `.github/actions/scanner-zap/action.yml`. Add a deprecation note pointing at the config-file path for any 0.6.8-shaped consumer workflows.~~ The seven inputs were already gone from `action.yml` (trimmed during the SDK refactor); this iteration adds the deprecation note + 0.6.x→1.x mapping table to the action's description block so an operator viewing the docsite or auto-generated workflow page gets pointed straight at `scanners.zap.*`. Audit gate `scripts/ci/check_example_inputs.py` (PR #136) confirms zero `examples/**` `with:` block still references the removed inputs.
  - [x] ~~Tests: `argus/tests/scanners/test_zap.py` covers config passthrough; integration test for env-var-referenced auth substitution.~~ Shipped alongside the wiring (PR #142): per-key tests for `api_spec` / `rules_file` / `cmd_options` / `max_duration_minutes` / `scan_type` / `auth.context_file`, plus credential-resolution coverage in `TestZapContainerEnv` exercising both registry and `auth.*` env-var name references.
...
- [x] ~~**(3) Cosign-verify scanner container images on pull.**~~ Shipped. New module `argus/core/image_verify.py` runs supply-chain verification on every image pull:
  ...
  - **Third-party tag-only pins** (most third-party images today): no cryptographic guarantee. Single WARNING per scan run via `report_tag_pinned_summary()` listing the unpinned images, with a hint to migrate to digest pins. **Migration shipped** — every entry in `argus/containers.py::OFFICIAL_IMAGES` now carries a `tag@sha256:...` reference, the `expected_version` parser handles the digest suffix, and the Renovate regex (`renovate.yaml`) was widened to capture an optional `currentDigest` so tag + digest stay in sync on every Renovate-driven bump. As a result the `report_tag_pinned_summary` warning is now silent in normal operation — argus-owned images go through cosign verify and every third-party image takes the `VERIFIED_DIGEST_PIN` path.
