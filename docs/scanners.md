...
| `M002` | Indirection (`@`) of a tainted expression | HIGH | CWE-94 (off by default) |
...
| `M201` | DO / GOTO to undeclared label | INFO | n/a (off by default) |
| `M202` | Routine name does not match filename | INFO | n/a |
| `M203` | Local variable read before it was defined | INFO | n/a (off by default) |
| `M204` | Local variable declared (NEW/READ) but never read | INFO | n/a (off by default) |
...
The security rules above cover all five MUMPS-specific code-injection sinks — XECUTE (M001), indirection (M002), OPEN/USE device arguments (M003), dynamic routine dispatch (M005), and external `$&` calls (M006) — plus data-at-rest credential leaks (M004). Together they exceed the public mHawk taint-sink surface for intra-procedural detection. The diagnostic rules (M101–M102, M201–M219) cover structural, control-flow (infinite FOR, value-QUIT-in-FOR), def-use, call-arity, scratch-global concurrency, VistA SAC, and portability checks — 21 diagnostics toward mHawk's ~32. Rules marked *off by default* (M002, M201, M203, M204, M205, M214, M216, M217) are opt-in via `rules.<id>.enabled: true`. M205/M214/M216/M217 are idiomatic in legacy VistA or portability-inventory only; M002 (indirection), M201 (undeclared label), M203 (read-before-def), and M204 (unused local) were measured **false-positive-dominant at scale** (see the precision note below) and await deeper analysis before they can ship on by default.

**Default profile precision (validated at scale).** The default profile is tuned so a clean codebase stays quiet and a real one yields actionable signal rather than a wall of noise. It was validated against **six real MUMPS projects totalling 1,162 routines** — a utility library, a MUMPS web server, two YottaDB projects, and the VistA **Kernel** (signon, device handling, `$ZF` callouts, the XECUTE-driven menu system) and Programmer Environment — with every distinct finding class adjudicated against the real source. The default profile produces **220 findings across all 1,162 routines** (down from 1,628 before precision tuning). The security signal is genuine: real shell-injection sinks in the Kernel (`OPEN "lsof"/"kill"/"ps":(shell=…:command=<tainted>)`, `$ZF(-1,…)` with a tainted command) are caught by M003/M006, and READ→XECUTE injection by M001. Four rules measured **false-positive-dominant at scale and now ship off by default** (opt-in via `rules.<id>.enabled: true`): **M002** indirection (fires on non-executable name/glvn indirection — `S @X=…`, `@X@(sub)`, `L +@X` — not just executable `X @v` / `D @v`; the executable cases are still covered by M001/M005), **M201** undeclared-label (the label extractor misses quote-heavy / final-block labels, so legal forward references read as undeclared), **M203** read-before-def (dynamic scoping + grammar misparse), and **M204** unused-local (intra-routine def-use cannot see a NEW-scoped var read by a callee through implicit-`NEW` inheritance).
...
- `rules.<id>.enabled` — turn an individual rule on or off. Most rules are on by default; **M002, M201, M203, M204, M205, M214, M216, and M217 ship off by default.** M002 (indirection), M201 (undeclared label), M203 (read-before-def), and M204 (unused local) were measured false-positive-dominant at scale (see *Default profile precision* above) and are opt-in audit passes pending deeper analysis. M205 (label fallthrough) is off because top-to-bottom fallthrough between labels is the intended idiom in old-style linear VistA routines (it was ~69% false-positive on the VistA Kernel). M214/M216/M217 are portability-inventory checks, opt-in for migration audits.
